...
class SVGRect:
	...
	def renderFromSVG(self, svgFName, sliceFName):
		rc = os.system('inkscape >/dev/null --without-gui --export-id="%s" --export-png="%s" "%s"' % (self.name, sliceFName, svgFName))
		if rc > 0:
...
if __name__ == '__main__':
	...
	if len(args) != 1:
		fatalError("Usage error.  SVGSlice must be called with the name of the input SVG file as the only argument.  If you're seeing this error within Inkscape, it probably means that the Inkscape extension protocol has changed, but SVGSlice hasn't been updated to match.")
